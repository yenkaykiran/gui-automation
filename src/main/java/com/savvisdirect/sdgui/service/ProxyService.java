...
public class ProxyService extends AbstractServiceImpl<Integer, Proxy, ProxyRepository> {
    ...
    public Proxy getFirst() {
        List<com.savvisdirect.sdgui.model.entity.Proxy> all = repository().findAll();
        com.savvisdirect.sdgui.model.entity.Proxy proxyDetails = null;
        if (all.size() > 0) {
            proxyDetails = all.get(0);
        } else {
            proxyDetails = new com.savvisdirect.sdgui.model.entity.Proxy();
        }
        return proxyDetails;
    }
